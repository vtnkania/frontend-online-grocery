...
export default function AdminFilterBar({ search, categoryId, storeId, stockStatus, options, onChange }: Props) {
  return (
    <div className="grid gap-3 rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200 lg:grid-cols-[1fr_180px_180px_160px]">
      <label className="flex items-center gap-3 rounded-lg border border-slate-200 px-4">
        ...
        <input value={search} onChange={(event) => onChange("search", event.target.value)} className="h-12 w-full bg-transparent outline-none" placeholder="Search by name, SKU or barcode..." />
      </label>
